...
app.post("/guild/:guildId/config", dashboardAuth, async (req, res) => {
  ...
  await updateConfig(guild, {
    ...
    spamTime: Number(req.body.spamTime || 8) * 1000,
    mentionLimit: Number(req.body.mentionLimit || 5),
    capsPercent: Number(req.body.capsPercent || 80),
    punishmentDuration: String(req.body.punishmentDuration || "10m"),
    raidJoinLimit: Number(req.body.raidJoinLimit || 5),
    raidJoinTime: Number(req.body.raidJoinTime || 10) * 1000,
  ...
  if (typeof sendLog === "function") {
    await sendLog(guild, "🌐 Dashboard Config Updated", "Config server diubah melalui dashboard web.", "Blue").catch(() => {});
  }
